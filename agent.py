...
class SacAgent:
    def __init__(self, env, log_dir, num_steps=3000000, batch_size=256,
                 ...
                 eval_interval=10, cuda=True, seed=0):
        self.env = env
        ...
        self.policy = GaussianPolicy(
            self.env.observation_space.shape[0],#17
            # 課題が持つ状態変数の数を示す
            self.env.action_space.shape[0], #3
        ...
        if entropy_tuning:
            ...
            self.target_entropy = -torch.prod(torch.Tensor(
                self.env.action_space.shape[0]).to(self.device)).item()
    ...
    def act(self, state):
        if self.start_steps > self.steps:
            #初期更新前のパラメータをどうするか。
            #action = (np.random.rand(self.env.action_space.shape[0]) - 0.5)/32 #一様に3cmが
    ...
    def train_episode(self):
        ...
        while not done:
            ...
            next_state, reward, done = self.env.step(action/10)
            self.steps += 1
            episode_steps += 1
        ...
        print('episode: {:4}'.format(self.episodes))
        print('episode steps: {:4}' .format(episode_steps))
        print('reward: {:5}' .format(episode_reward))
    ...
    def evaluate(self):
        ...
        print('-' * 60)
        print('Num steps:{:<5}' .format(self.steps))
        print('reward: {:<5.1f}'.format(mean_return))
        print('-' * 60)
